...
- **Budget** (NFR-505): ≤ 4 KB gzipped added to the app payload of commit `0a4194f`; zero extra requests; zero new runtime asset files. Measured delta at ship: **0.94 KiB** gzipped on linux zlib-ng (slightly negative on macOS stock zlib). Amended by spec-06: that result is immutable rather than charged for later features; VC-513 retains live switch latency, long-task, zero-request, emitted-file-set, and manifest-count checks.
